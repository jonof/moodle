...
Y.extend(DRAGBLOCK, M.core.dragdrop, {
    ...
    initializer : function() {
        ...
        var protectedregionnames = this.get('protectedregions');

        blockregionlist.each(function(blockregionnode) {
            if (protectedregionnames.indexOf('side-'+this.get_region_id(blockregionnode)) >= 0) {
                return;
            }
...
MANAGER.prototype = {
    ...
    initializer : function() {
        ...
        var regionnames = this.get('regions'),
            protectedregionnames = this.get('protectedregions'),
            i = 0,
            region,
            regionname,
            droptarget,
        ...
        for (i in regionnames) {
            ...
            if (protectedregionnames.indexOf(regionname) >= 0) {
                // Don't set up block regions for drag-drop when they are protected.
                continue;
            }
...
Y.extend(MANAGER, M.core.dragdrop, MANAGER.prototype, {
    ...
    ATTRS : {
        ...
        /**
         * An array of block regions that are protected from use based on user privilege.
         * @attribute protectedregions
         * @type array|null
         * @default Array[]
         */
        protectedregions : {
